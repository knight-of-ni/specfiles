...
# There are several files in the tarball that have the executable bit set that shouldn't
find . -type f ! \( -name 'configure' -or -name '*.sh' \) -executable -print -exec chmod -x '{}' \;
...
# Remove duplicate manpage files in the docs subfolder
rm -f -- doc/*.1
rm -f -- doc/*.3

%check
# nothing to do, no tests available
...
%changelog
* Fri May 30 2025 Andrew Bauer <zonexpertconsulting@outlook.com> - 2.4.6-1
- 2.4.6 release
